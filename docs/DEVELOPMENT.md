...
Флаг `DEBUG_TOOLS` (`src/config.ts`, по умолчанию включён во всех сборках):

| Клавиша            | Функция                                |
| ------------------ | -------------------------------------- |
| `F1`               | счётчик FPS (состояние в localStorage) |
| `F2`               | хитбоксы сущностей                     |
| `F3`               | замедление игры ×0.25                  |
| `F4`               | бессмертие (жизни не тратятся)         |
| `+` / `-` (numpad) | зачистка уровня / урон шара            |

Плюс панель «🐛 Отладка» в меню (спавн боссов/минибоссов, режимы ракетки).
В консоль пишется лог переключений (`[ШАРОБОЙ][debug] …`).

**Урезание отладки в стабильной версии:** передать в сборку
`VITE_DEBUG_TOOLS=0` — клавиши, панель и индикаторы вырезаются из бандла,
код менять не нужно. Для деплоя из `main` с урезанной отладкой добавьте
`VITE_DEBUG_TOOLS: "0"` в env-блок шага build (`GH_PAGES_BASE: /SharoBoy/`)
в `.github/workflows/deploy.yml`; бета (`/SharoBoy/beta/`) собирается
с полной отладкой.
...
- неигровая HUD-зона сверху существует только на вертикальных экранах:
  `Game.blockTop()` = нижняя граница HUD-плашек (`hudTopCss()`, CSS px:
  `HUD_TOP_PORTRAIT_CSS` — там HUD двухрядный), делённая на `scale`, с клампами
  14..35% высоты мира; в ландшафте `blockTop = 0` — шар отскакивает от верхней
  кромки, блоки не клампятся (визуальная подсказка `drawTopZone` тоже скрыта).
  При смене ориентации посреди уровня `resizeHandler` вызывает
  `game/rotateLayout.ts#realignOnOrientationChange`: блоки и босс единым сдвигом
  опускаются ниже новой зоны (орбитальные/бомбовые не двигаются — их ведёт
  босс); в ландшафт ничего не сдвигается; вне `playing`/`paused` — тоже;
...
| `20250101000007_grants_api_access.sql`      | Явные GRANT для Data API (с 30.10.2025 Supabase не выдаёт их автоматически) |
...
**Правило GRANT (с 30.10.2025):** Supabase больше не выдаёт Data API-доступ
новым таблицам в `public` автоматически. Любая новая таблица обязана получать
явные `grant ... to anon / authenticated / service_role` в той же миграции,
что и `create table` (см. `20250101000007_grants_api_access.sql`). Симптом
пропуска: permission denied от PostgREST/Edge Function на свежем проекте,
preview-ветке или после `supabase db reset` — боевой базе при этом ничего
не грозит (старые гранты сохраняются).
