...
- зона блоков стартует ниже HUD-плашек: высота неигровой зоны (`hudTopCss()`,
  CSS px: `HUD_TOP_CSS` в ландшафте, `HUD_TOP_PORTRAIT_CSS` в портрете — там
  HUD двухрядный) переводится в мировые единицы делением на `scale`
  (`Game.blockTop()`);
