...
export function signScore(nick: string, score: number, mode: "campaign" | "endless", wave: number): string {
  return fnv1a(`${nick}:${score}:${mode}:${wave}:${SCORE_SECRET}`);
}

/** Проверяем подпись строки результата (без сигнатуры или с битой — мимо). */
function isSigValid(row: GlobalScore, mode: "campaign" | "endless"): boolean {
