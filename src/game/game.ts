...
function fitTilt(rx: number, ry: number, halfW: number, halfH: number) {
...
const TIER: Record<1 | 2 | 3, { base: string; light: string; dark: string }> = {
...
const ARROW_LAYOUT: LayoutItem[] = [
  ...
  ...Array.from({ length: 13 }, (_, i): LayoutItem => ({ x: -3.6 + i * 0.6, y: 0.55, rx: 0.3, hp: 1 })),
...
export class Game {
  ...
  private buildFromSpec(spec: LevelSpec) {
    ...
    if ("layout" in spec) {
      ...
      for (const it of spec.layout) {
        ...
        const cx = clamp(this.w / 2 + offsetX + it.x * unit, margin * 0.5 + rx, this.w - margin * 0.5 - rx);
        blocks.push({
          x: cx,
          y: top + it.y * unit,
          rx,
  ...
  private updateBall(ball: Ball, dt: number) {
    ...
    const mult =
      (this.time < this.slowUntil ? 0.72 : this.time < this.fastUntil ? 1.32 : 1) * ramp;
  ...
  private damageBlock(b: Block, dmg = 1) {
    ...
    this.rings.push({ x: b.x, y: b.y, r: 6, maxR: 40 + b.tier * 18, color: "rgba(234,247,255,0.7)", t: 0 });
    ...
    if (this.combo === 5 || this.combo === 10 || this.combo === 15) {
      ...
      this.popups.push({ x: this.w / 2, y: this.h * 0.3, text: `${word} ×${this.combo}`, color: "#ffc94d", t: 0, size: 30 });
      this.rings.push({ x: this.w / 2, y: this.h * 0.3, r: 10, maxR: 150, color: "rgba(255,201,77,0.6)", t: 0 });
      this.sfx.levelClear();
    }
  ...
  private tryFieldShift(dt: number) {
    ...
    this.popups.push({ x: this.w / 2, y: this.h * 0.25, text: "СДВИГ ПОЛЯ", color: "#9fd6ea", t: 0, size: 20 });
  ...
  private updatePowers(dt: number) {
    ...
    this.powers = this.powers.filter((pw) => !(pw as PowerUp & { taken?: boolean }).taken && pw.y < this.h + 40);
  }
  ...
  private applyPower(type: PowerType) {
    ...
    const popup = (text: string) =>
      this.popups.push({ x: this.paddle.x, y: this.paddle.y - 40, text, color: meta.color, t: 0, size: 18 });
    switch (type) {
      case "wide":
