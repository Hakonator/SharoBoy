/**
 * Фильтр запрещённых ников — общий для клиента и Edge Function «scores».
 *
 * Файл чистый (без Deno- и браузерных API): клиент ре-экспортирует его через
 * src/game/profanity.ts, функция «scores» валидирует ники перед записью в топ,
 * поэтому обходы (мат, leet, разделители) отсекаются и на сервере.
 *
...
/** Максимальная длина ника — единая для клиента и сервера (совпадает с maxLength в UI). */
export const MAX_NICK = 16

export function validateNick(raw: string): NickCheck {
  // Убираем пробелы по краям и схлопываем множественные пробелы,
  // но не трогаем другие whitespace (\t, \n…) — они не пройдут валидацию
  const nick = raw.trim().replace(/ +/g, " ")
  if (nick.length < 2) {
    return { ok: false, nick, error: "Ник слишком короткий — минимум 2 символа" }
  }
  if (nick.length > MAX_NICK) {
    return { ok: false, nick, error: `Ник слишком длинный — максимум ${MAX_NICK} символов` }
  }
